//! HTTP implementation for Datadog V2 metrics intake API.
//!
...
/// Run the HTTP V2 metrics server
///
/// # Errors
///
/// Function will return an error if the server fails to start.
pub(crate) async fn run_v2_server(
    binding_addr: SocketAddr,
    metric_labels: Vec<(String, String)>,
    shutdown: lading_signal::Watcher,
) -> Result<(), super::Error> {
    let metric_labels: Arc<[(String, String)]> = Arc::from(metric_labels);
    let state = Arc::new(AppState { metric_labels });

    let listener = TcpListener::bind(binding_addr).await?;
    info!(
        "Datadog V2 metrics intake blackhole listening on {}",
        binding_addr
    );
