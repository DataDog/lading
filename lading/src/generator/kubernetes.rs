    ...
    pub async fn spin(self) -> Result<(), Error> {
        ...
        macro_rules! spin {
            (namespaced, $type:ty, $object:expr) => {{
                ...
                let api: kube::Api<$type> = kube::Api::namespaced(client, &namespace);
                spin_inner!($type, $object, api);
            }};
            (cluster_level, $type:ty, $object:expr) => {{
                let api: kube::Api<$type> = kube::Api::all(client);
                spin_inner!($type, $object, api);
            }};
        }
                ...
                for i in 0..self.number_of_exemplars.into() {
                    ...
                    set.spawn(async move {
                        match api.create(&kube::api::PostParams::default(), &object).await {
                            Ok(o) => debug!("Created resource: {o:?}"),
                            Err(e) => warn!("Failed to create resource: {e}"),
                        }
                    });
                }
        ...
        match self.resource {
            ...
            Resource::Pod(pod) => spin!(namespaced, k8s_openapi::api::core::v1::Pod, pod),
