//! Attribute generator for OpenTelemetry metrics payloads
use opentelemetry_proto::tonic::common::v1::{AnyValue, KeyValue, any_value};

use crate::common::{config::ConfRange, strings::Pool};
use std::rc::Rc;
...
impl Generator {
    /// Creates a new attribute generator
    ///
    /// # Errors
...
impl<'a> crate::Generator<'a> for Generator {
        ...
        R: rand::Rng + ?Sized,
    {
        ...
        let mut kvs = Vec::with_capacity(tag_handles.len());
        for crate::common::tags::Tag { key, value } in tag_handles.drain(..) {
            ...
            kvs.push(KeyValue {
                key: String::from(key_s),
                value: Some(AnyValue {
                    value: Some(any_value::Value::StringValue(String::from(val_s))),
                }),
            });
        }
        Ok(kvs)
