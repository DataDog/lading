...
impl StaticTimestamped {
    ...
    /// the message. `start_line_index`, when provided, skips that many lines
    /// (modulo the total number of available lines) before returning payloads.
    ///
    /// # Errors
    ///
    ...
    fn parse_line_with_format(line: &str, timestamp_format: &str) -> Result<(i64, Vec<u8>), Error> {
        ...
        let (ts, payload) = NaiveDateTime::parse_and_remainder(line, timestamp_format)
            .map_err(|_| Error::Timestamp(line.to_string()))?;
        let payload = payload.trim_start().as_bytes().to_vec();
