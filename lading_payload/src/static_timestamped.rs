...
impl StaticTimestamped {
    ...
    /// timestamp is parsed from the start of the line using
    /// `timestamp_format` (chrono strftime syntax), and the parsed timestamp
    /// prefix is removed from the emitted line, leaving only the remainder of
    /// the message. `start_line_index`, when provided, seeks to that line
    /// (modulo the total number of available lines) and then backtracks to the
    /// start of the timestamp group it falls in, so the first block always
    /// contains a complete second.
    ///
    /// # Errors
    ///
    ...
    pub fn new(
        ...
        if timestamp_format.contains("%Z") {
            return Err(Error::UnsupportedTimezoneFormat);
        }
        ...
        // If a start line index was given, backtrack to the start of the
        // timestamp group it falls in — same behaviour as the random-start path.
        if let Some(idx) = start_line_index {
            let raw_offset = idx % total_lines;
            let group_offset = instance.find_group_start(raw_offset)?;
            instance.initial_offset = Some(group_offset);
            instance.reset_reader(group_offset)?;
        }
    ...
    fn parse_line_with_format(line: &str, timestamp_format: &str) -> Result<(i64, Vec<u8>), Error> {
        ...
        // Try timezone-aware parsing first (%z, %:z, %+), then fall back to
        // naive (treated as UTC) for formats without an offset.
        //
        // Strip at most one leading whitespace character from the remainder to
        // consume the separator between the timestamp and the message body,
        // while preserving any intentional leading whitespace in the message.
        let (sec, payload) = if let Ok((ts, remainder)) =
            DateTime::<FixedOffset>::parse_and_remainder(line, timestamp_format)
        {
            let body = remainder
                .strip_prefix(|c: char| c.is_ascii_whitespace())
                .unwrap_or(remainder);
            (ts.timestamp(), body.as_bytes().to_vec())
        } else {
        ...
        loop {
            ...
            match current_second {
                ...
                Some(s) if s < line_sec => {
                    ...
                    const LARGE_GAP_WARNING_THRESHOLD: u64 = 3_600; // 1 hour
                    if self.emit_placeholder && line_sec > s + 1 {
                        let gap = u64::try_from(line_sec - s - 1).unwrap_or(0);
                        if gap > LARGE_GAP_WARNING_THRESHOLD {
                            warn!(
                                gap_seconds = gap,
                                "StaticTimestamped detected a large timestamp gap; \
                                 a placeholder block will be emitted for each missing second. \
                                 Consider setting emit_placeholder=false for sparse files."
                            );
                        }
        ...
        W: Write,
    {
        ...
        if block.lines.is_empty() {
            ...
            for line in &block.lines {
                ...
                if needed > max_bytes {
                    // This individual line is larger than the budget. Skip it
                    // so we don't produce an empty block and hang cache
                    // construction. Warn so the user knows to raise
                    // maximum_block_bytes.
                    warn!(
                        max_bytes,
                        line_bytes = needed,
                        "StaticTimestamped line exceeds max_bytes and will be skipped. \
                         Increase maximum_block_bytes in your configuration to avoid dropping lines."
                    );
                    continue;
                }
...
impl From<Error> for crate::Error {
    fn from(err: Error) -> Self {
        match err {
            ...
            | Error::Timestamp(_)
            | Error::OutOfOrder { .. }
            | Error::UnsupportedTimezoneFormat => crate::Error::Validation(err.to_string()),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn parses_timestamps_with_spaces_in_format() {
        let temp_dir = tempdir().unwrap();
        let path = temp_dir.path().join("space_timestamp_format.log");
        {
            let mut f = File::create(&path).unwrap();
            writeln!(f, "2024-01-01 00:00:00 first").unwrap();
            writeln!(f, "2024-01-01 00:00:01 second").unwrap();
        }

        let mut serializer =
            StaticTimestamped::new(&path, "%Y-%m-%d %H:%M:%S", false, Some(0)).unwrap();
        let mut rng = StdRng::seed_from_u64(7);
        let mut buf = Vec::new();
    ...
    #[test]
    fn rejects_percent_z_format() {
        let temp_dir = tempdir().unwrap();
        let path = temp_dir.path().join("tz_abbrev.log");
        {
            let mut f = File::create(&path).unwrap();
            writeln!(f, "2024-01-01 00:00:00 PST first").unwrap();
        }

        let err = StaticTimestamped::new(&path, "%Y-%m-%d %H:%M:%S %Z", false, None).unwrap_err();
        assert!(
            matches!(err, Error::UnsupportedTimezoneFormat),
            "expected UnsupportedTimezoneFormat, got {err}"
        );
    }
}
