...
//! parsing a timestamp at the start of each line.
...
impl StaticSecond {
    ...
    /// whitespace, using `timestamp_format` (chrono strftime syntax).
    pub fn new(path: &Path, timestamp_format: &str, emit_placeholder: bool) -> Result<Self, Error> {
        ...
        for line_res in reader.lines() {
            ...
            // Take prefix until first whitespace as the timestamp segment.
            let ts_token = line.split_whitespace().next().unwrap_or("");
            ...
            match current_sec {
                Some(s) if s == sec => {
                    current_lines.push(line.as_bytes().to_vec());
                }
                Some(s) if s < sec => {
                    ...
                    current_lines = vec![line.as_bytes().to_vec()];
