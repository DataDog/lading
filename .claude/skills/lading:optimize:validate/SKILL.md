...
Optimization hunt discovers bug
    ...
    /lading:optimize:validate
            ...
            +-- Validate fix works
            +-- Record as beads issue with opt-validate label
            |
            v
...
### 5.2 MANDATORY: Create beads issue

```bash
bd create --type=bug --labels=opt-validate \
  --title="Bug: <file:function> - <short description>" \
  --description="$(cat <<'EOF'
## Bug Validation
...
- **Discovered By**: <hunt|rescue|review>
...
## Tests Added
- <test name> (proptest|kani proof)
- ...

## Verification
- Test fails before fix: YES
- Test passes after fix: YES
- ci/validate passes: YES
...
- [ ] Recorded as beads issue with `opt-validate` label
