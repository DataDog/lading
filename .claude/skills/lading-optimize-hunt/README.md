...
**Key principles**:
1. **Measure first**: Use profiling to identify hot paths
2. **One change at a time**: Isolate the effect of each optimization
3. **Benchmark everything**: Both micro and macro benchmarks must show improvement
4. **Preserve correctness**: Always run `ci/validate` and check determinism

**Remember**: Not every optimization applies to every situation. Cold paths don't need optimization. Hot paths with wrong optimizations can make things worse. Always benchmark.
