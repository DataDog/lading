...
### Phase 8: Beads Access

```bash
echo "=== Phase 8: Beads Access ==="
which bd > /dev/null 2>&1 && echo "bd command: available" || echo "FAIL: bd command not found"
bd list --status=all > /dev/null 2>&1 && echo "beads database: accessible" || echo "FAIL: beads database not accessible"
```
**Expected:** bd command available and database accessible
**If fails:** Check beads installation, run `bd init` if needed
...
Failed checks:
  ...
  [X] payloadtool --memory-stats - not supported
      Fix: cargo build --release --bin payloadtool
